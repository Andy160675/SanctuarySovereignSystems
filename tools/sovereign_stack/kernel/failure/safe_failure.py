...
class FailureClass(Enum):
    ...
    PROCESS_BLOCK = auto()      # Operation bypassed a gate â€” halt operation
    TRANSACTION_HOLD = auto()   # Authority violation â€” hold in escrow
    KERNEL_PANIC = auto()       # Axiom violation â€” system-wide halt
...
class SafeFailure:
    ...
    All failures result in explicit halt states â€” never silent pass-through.
    """
    ...
    def kernel_panic(self, component: str, reason: str, signal_id: str) -> FailureRecord:
        ...
        self._halted = True
        self._halt_reason = f"KERNEL_PANIC: {component} â€” {reason}"
        self._record_failure(record)
        return record
    ...
    def safe_execute(
        ...
        **kwargs: Any,
        ...
        On exception â†’ process_block, return None.
        """
        if self._halted:
    ...
    def _record_failure(self, record: FailureRecord) -> None:
        """Log the failure â€” Axiom I: immutable evidence."""
