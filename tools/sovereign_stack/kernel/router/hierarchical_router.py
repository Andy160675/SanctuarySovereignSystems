...
class HierarchicalRouter:
    ...
    def set_system_state(self, state: SystemState) -> None:
        """Explicit state transition â€” no silent drift."""
    ...
    def route(self, signal: TypedSignal) -> RouteResult:
        ...
        """
        # Step 1: Legality Lane â€” Article III
        ...
        # Step 2: Determine routing authority â€” Article IV + V
        ...
        if handler is None:
            # Axiom II: Ambiguity â†’ Containment. No handler = halt.
            result = RouteResult(
                signal=signal,
                routed_to=target_authority,
                handler_name="NONE",
                accepted=False,
                reason="transaction_hold: no registered handler, ambiguity â†’ containment",
            )
            self._log_route(result)
            return result
        ...
        try:
            ...
            result = RouteResult(
                ...
                handler_name=handler_name,
                accepted=False,
                reason=f"kernel_panic: handler exception â€” {str(e)}",
            )
    ...
    def _resolve_authority(self, signal: TypedSignal) -> AuthorityLevel:
        ...
        stable â†’ operator, degraded â†’ innovator, constitutional â†’ steward.
