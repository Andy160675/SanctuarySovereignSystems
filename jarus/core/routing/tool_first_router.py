...
# UI/Visual patterns
UI_PATTERNS = [
    r'\b(ui|interface|dashboard|screen|screenshot|button|menu|icon)\b',
    r'\b(rendering|layout|pixel|color|font|css|html)\b',
    r'\b(user\s+experience|ux|ui\s+debug)\b',
]

# Log/Technical patterns
LOG_PATTERNS = [
    r'\b(logs?|parsing|parse|stderr|stdout|exception|stacktrace)\b',
    r'\b(error|warn|info|debug|critical)\b',
    r'\b(event|audit|ledger)\b',
]

# Data Extraction patterns
EXTRACTION_PATTERNS = [
    r'\b(extract|pull|get|fetch)\b',
    r'\b(data|fields?|values?|entities)\b',
    r'\b(json|xml|csv|table)\b',
]
...
class ToolFirstRouter:
    ...
    def _compile_patterns(self):
        ...
        # Combine all patterns for each tool type into a single regex
        self.patterns = {
            ToolType.CALCULATOR: re.compile('|'.join(f'(?:{p})' for p in MATH_PATTERNS), re.IGNORECASE),
            ToolType.DATE_TIME: re.compile('|'.join(f'(?:{p})' for p in DATE_PATTERNS), re.IGNORECASE),
            ToolType.RULES_DATABASE: re.compile('|'.join(f'(?:{p})' for p in RULES_PATTERNS), re.IGNORECASE),
            ToolType.ENTITY_LOOKUP: re.compile('|'.join(f'(?:{p})' for p in ENTITY_PATTERNS), re.IGNORECASE),
            ToolType.RATE_LOOKUP: re.compile('|'.join(f'(?:{p})' for p in RATE_PATTERNS), re.IGNORECASE),
            ToolType.UI_ANALYSIS: re.compile('|'.join(f'(?:{p})' for p in UI_PATTERNS), re.IGNORECASE),
            ToolType.LOG_PARSING: re.compile('|'.join(f'(?:{p})' for p in LOG_PATTERNS), re.IGNORECASE),
            ToolType.DATA_EXTRACTION: re.compile('|'.join(f'(?:{p})' for p in EXTRACTION_PATTERNS), re.IGNORECASE),
        }
    ...
    def route(self, query: str) -> RoutingDecision:
        ...
        for tool_type, pattern in self.patterns.items():
            tool_matches = pattern.findall(query)
