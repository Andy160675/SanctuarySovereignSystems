...
def write_sitrep_md(sitrep_path: Path, statuses: list[NodeStatus]) -> None:
    # Read existing content to preserve Seal manifest and Version lines
    preserved_lines = []
    if sitrep_path.exists():
        try:
            for line in sitrep_path.read_text(encoding="utf-8").splitlines():
                l_strip = line.strip().lower()
                if l_strip.startswith("- seal manifest:") or l_strip.startswith("version:"):
    ...
    # Add preserved Version line if it was near the top
    for pl in preserved_lines:
        if pl.strip().lower().startswith("version:"):
            lines.append(pl)
            break
    ...
    # Add preserved Seal manifest line
    for pl in preserved_lines:
        if pl.strip().lower().startswith("- seal manifest:"):
            lines.append(pl)
            break
