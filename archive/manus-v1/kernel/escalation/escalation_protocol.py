...
class EscalationDecision:
    """Record of an escalation decision — immutable evidence."""
...
class EscalationProtocol:
    ...
    State transitions follow strict rules — no intuition permitted.
    """
    ...
    def escalate(
        ...
        if target_state not in valid_targets:
            decision = EscalationDecision(
                ...
                to_state=self._current_state,  # No change — containment
                trigger_signal_id=trigger.signal_id,
                resolved_authority=self.current_authority,
                reason=f"BLOCKED: invalid transition {self._current_state.value} → {target_state.value}",
            )
        else:
    ...
    def de_escalate(self, trigger: TypedSignal, reason: str) -> EscalationDecision:
        """
        Step down one level. Constitutional → Degraded → Stable.
